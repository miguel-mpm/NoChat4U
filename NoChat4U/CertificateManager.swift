...
struct CertificateManager {
    ...
    static func fetchCertificate() async throws -> (certificate: NIOSSLCertificate, privateKey: NIOSSLPrivateKey) {
        ...
        do {
            ...
            guard let cert = try? NIOSSLCertificate(bytes: [UInt8](certData), format: .pem),
    ...
    private static func certExpiry(from pemData: Data) -> Date? {
        ...
        guard let values = SecCertificateCopyValues(secCert, keys, nil) as? [String: Any],
              ...
              let notAfter = notAfterDict[kSecPropertyKeyValue as String] as? Date else { return nil }
